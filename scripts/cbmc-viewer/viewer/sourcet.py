...
def build_with_preprocessor(directory=None):
    ...
    # strip line continuations from make output
...
def build_source_files(output, directory=None):
    """Extract source file names from preprocessor output."""

    # extract linemarkers from preprocessor output
    # linemarkers have the form '# linenum filename flags' (space after #)
    # directives have the form '#directive' (no space after #)
    linemarkers = [line for line in output if line.strip().startswith('# ')]

    # extract filenames from linemarkers (and strip enclosing double quotes)
    # warning: split won't work on windows: filenames may constain whitespace
    filenames = [line.split()[2].strip('"') for line in linemarkers]

    # skip filenames generated by goto-cc for built-in functions, etc.
...
def sloc(files, root=None):
    ...
    try:
        ...
        logging.debug("Unable to run sloc: %s", ' '.join(command))
        ...
        logging.debug("Unable to run sloc: %s", ' '.join(error.cmd))
        logging.info("Unable to run sloc: stderr: %s", error.stderr)
        return None
